...
const completed = await client.post(`/api/checkout_sessions/${id}/complete`, {})
check('refused with 501', completed.status === 501, completed.status)
check('names the missing gate', completed.json?.error?.code === 'gate_not_implemented')

console.log('terminal states')
const canceled = await client.post(`/api/checkout_sessions/${id}/cancel`, {})
check('canceled', canceled.json?.status === 'canceled', canceled.json?.status)
const again = await client.post(`/api/checkout_sessions/${id}/cancel`, {})
check('second cancel refused', again.json?.error?.code === 'session_terminal', again.json?.error)
const mutate = await client.post(`/api/checkout_sessions/${id}`, { items: [{ product_id: 'sku_chai_250', quantity: 1 }] })
